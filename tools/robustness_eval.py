...
def get_coco_style_results(
    ...
    if metric is None:
        ...
        if "rPC" in prints:
            print("Realtive Performance under Corruption [rPC] ({})".format(task))
        ...
        if "rPC" in prints:
            print("Relative Performance under Corruption [rPC] ({})".format(task))
            for metric_i, metric_name in enumerate(metrics):
...
def get_voc_style_results(filename, prints="mPC", aggregate="benchmark"):
    ...
    if "P" in prints:
        print(
            "{:48} = {:0.3f}".format(
                "Performance on Clean Data [P] in AP50", np.mean(P)
            )
        )
    if "mPC" in prints:
        print(
    ...
    if "rPC" in prints:
        print(
            "{:48} = {:0.1f}".format(
                "Realtive Performance under Corruption [rPC] in %", np.mean(rPC) * 100
            )
        )
